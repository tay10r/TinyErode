...
SimState::HeightData(const uint8_t* data)
{
  ...
  for (int i = 0; i < numCells; i++) {
    tmp[i] = (data[i] / 255.0F) * m_config.maxElevation;
  }
...
SimState::HeightData(const uint16_t* data)
{
  ...
  for (int i = 0; i < numCells; i++) {
    tmp[i] = (data[i] / 65535.0F) * m_config.maxElevation;
  }
...
SimState::HeightNoise(FastNoiseLite& noise, const float distancePerCell)
{
  ...
  for (int i = 0; i < numCells; i++) {
    ...
    data[i] = std::max(noise.GetNoise(u * aspect * distancePerCell, v * distancePerCell), 0.0F) * m_config.maxElevation;
  }
